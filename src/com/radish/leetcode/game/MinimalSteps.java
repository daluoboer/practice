...
/**
 * @description MinimalSteps
 *
 * LCP 13. 寻宝
 * 我们得到了一副藏宝图，藏宝图显示，在一个迷宫中存在着未被世人发现的宝藏。
 *
 * 迷宫是一个二维矩阵，用一个字符串数组表示。它标识了唯一的入口（用 'S' 表示），和唯一的宝藏地点（用 'T1' 表示）。但是，宝藏被一些隐蔽的机关保护了起来。在地图上有若干个机关点（用 'M' 表示），只有所有机关均被触发，才可以拿到宝藏。
 *
 ...
 * 输出：16
 *
 * 解释：最优路线为： S->O, cost = 4, 去搬石头 O->第二行的M, cost = 3, M机关触发 第二行的M->O, cost = 3, 我们需要继续回去 O 搬石头。 O->第三行的M, cost = 4, 此时所有机关均触发 第三行的M->T1, cost = 2，去T点拿宝藏。 总步数为16。 图片.gif
 *
 * 示例 2：
 *
 * 输入： ["S#O", "M.#", "M.T1"]
...
public class MinimalSteps {
    ...
    int[] dy = {0,0,1,-1};
    int n,m;
    //规则明白了，但是不会算啊，有点难受......就是很丧，刚才跟亚敏说了明天来，然后改签了高铁，后天就去山东了，周五
